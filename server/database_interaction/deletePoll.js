module.exports = function(req, res, username, pollID) {
    ...
    // Bad input, missing required information
        console.log('Error, tried to delete poll with insufficient information');
        res.json({
            'Err': 'Incorrect Information sent for deletion request'
        });
    } else {
        var Entry = require('./Entry');
        // pollID and username have to match to remove poll
        Entry.findOneAndRemove({
            ...
            if (err) {
                ...
                if (doc == null) {
                    ...
                    console.log('Error, Poll ' + pollID + ' request for deletion could not be completed as poll does not exist');
                    res.json({
